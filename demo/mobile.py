...
class Mobile(object):
    ...
    def push_movies_to_mobile(self,local_file_path, remote_file_path):


        self.adb.shell("mkdir -p  {}".format(remote_file_path))

        for filename in os.listdir(local_file_path):
            src = "{}/{}".format(local_file_path, filename)
            dst = "{}/".format(remote_file_path)
    ...
    def push_movie_to_douyin(self,filename,local_file_path):
